...
class TestsAssetsService(TypeChecker):
    def test_get_exchanges(self, sync_client):
        res = sync_client.get_exchanges()
        for e in res.exchanges:
            assert isinstance(e.mic, str)
            assert isinstance(e.name, str)
        assert len(res.exchanges) > 0

    def test_get_assets(self, sync_client):
        res = sync_client.get_assets()
        for a in res.assets:
    ...
    def test_get_asset_params(self, sync_client, account_id):
        assets = sync_client.get_assets()
        symbol = choice(assets.assets).symbol
        res = sync_client.get_asset_params(symbol, account_id)
    ...
    def test_get_asset_params_negative(self, sync_client, account_id):
        symbol = "LKOH"
        with pytest.raises(RpcError) as exc:
            sync_client.get_asset_params(symbol, account_id)
        assert "Mic must not be empty" in exc.value.details()

    def test_get_options_chain(self, sync_client):
        symbol = "LKOH@RTSX"
    ...
    def test_get_options_chain_negative(self, sync_client):
        symbol = "asdasd@RTSX"
        res = sync_client.get_options_chain(symbol)
        assert len(res.options) == 0
    ...
    def test_get_schedule(self, sync_client):
        assets = sync_client.get_assets()
        symbol = choice(assets.assets).symbol
        res = sync_client.get_schedule(symbol)
        assert isinstance(res.symbol, str)
    ...
    def test_get_clock(self, sync_client):
        lag = 100  # Максимальное допустимое различие во времени
        res = sync_client.get_clock()
        assert abs(res.timestamp.ToMilliseconds() - time.time() * 1000) < lag

    def test_get_asset(self, sync_client, account_id):
        assets = sync_client.get_assets()
        symbol = choice(assets.assets).symbol
        res = sync_client.get_asset(symbol, account_id)
